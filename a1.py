...
def bfs(graph, root, max_depth):
    ...
    d = deque([root])
    while d:
        every_node = d.popleft()
        for n in graph.neighbors(every_node):
            if( n not in check):
                if(max_depth > node2distances[every_node]):
                    check.append(n)
                    d.append(n)
                    node2num_paths[n] = 1
                    node2distances[n] = node2distances[every_node] + 1
                    node2parents[n].append(every_node)
            else:
                if(node2distances[n] == node2distances[every_node] + 1):
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    keys = [n for n,v in node2distances.items()]
    measure={}
    ...
    sort_Nodes = sorted(node2distances.items(), key=lambda x:(x[1]), reverse = True)
    for n,v in sort_Nodes:
        if(node2num_paths[n] == 1):
            for parent in node2parents[n]:
                  measure[parent] = measure[parent] + measure[n]
        elif(node2num_paths[n] > 1):
            for ele in node2parents[n]:
                measure[ele] = measure[ele] + (1.0/node2num_paths[n])
                measure[n] = 1.0/node2num_paths[n] 
            
                    
    q={}
    
    nodekeys = [n for n,v in measure.items()]

    for n in nodekeys: 
        for parent in node2parents[n]:
            temp = (n,parent)
            tup = sorted(temp)
            q[(tup[0],tup[1])]=measure[n]
    
    
    ans = sorted(q.items(), reverse = True)
    return q
...
def approximate_betweenness(graph, max_depth):
    ...
    for n in nodes:
        ...
        for k,z in result.items():
            F_Result[k] = F_Result[k] + z
    
    keys = [k for k,z in F_Result.items()]
    for k in keys:
        F_Result[k] = F_Result[k]/2
...
def get_components(graph):
    ...
    """
    return [compo for compo in nx.connected_component_subgraphs(graph)]
...
def partition_girvan_newman(graph, max_depth):
    ...
    approx_bet=approximate_betweenness(graph,max_depth)
    approx_bet=sorted(approx_bet.items(),key=lambda x:x[1], reverse=True)

    
    Gp.remove_edge(*(approx_bet[counter][0]))
    sub_graphs =[x for x in nx.connected_component_subgraphs(Gp)]
    n = len(sub_graphs)
...
"""
    ...
    graph_so = nx.Graph()
    graph_de = graph.degree()
    
    for d in graph_de:
       
        if( min_degree <= graph_de[d[0]] ):
            graph_so.add_node(d[0])
    return graph.subgraph(graph_so)
...
def norm_cut(S, T, graph):
    ...
    nor_cut= ((cut(S, T, graph)/float(volume(S, graph))) + (cut(S, T, graph)/float(volume(T, graph))))
    return nor_cut
...
def brute_force_norm_cut(graph, max_size):
    ...
    for max_depth in max_depths:
        ...
        vadadu = partition_girvan_newman(graph, max_depth)
        S = list(vadadu[0].nodes())
        T = list(vadadu[1].nodes())
        cut = 0
        for y in graph.edges():
             if((y[0] in S and y[1] in T) or (y[0] in T and y[1] in S)):
                 cut += 1
                 x.append((y[0],y[1]))
        if(cut <= max_size):
             ncut=((cut/float(volume(S, graph))) + (cut/float(volume(T, graph))))
        #ncut = norm_cut(A, B, graph)
             tuples.append((ncut, x))
        x=[]
...
def score_max_depths(graph, max_depths):
    ...
    for max_depth in max_depths:
        ...
        vadadu = partition_girvan_newman(graph, max_depth)
        A = list(vadadu[0].nodes())
        B = list(vadadu[1].nodes())
        ncut = norm_cut(A, B, graph)
        tuples.append((max_depth, ncut))
...
def make_training_graph(graph, test_node, n):
    ...
    Graph_2 = graph.copy()
    counter = 1
    for e in sorted(Graph_2.edges(test_node)):
        if(counter <= n):
            counter = counter + 1
            Graph_2.remove_edge(*e)
        elif(counter > n):
            break
    return Graph_2
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        if n != node and not graph.has_edge(node, n):
            neighj = set(graph.neighbors(n))
            score = 1. * (len(neigh & neighj)) / (len(neigh | neighj))
            scores.append(((node, n), score))
    
    s_Scores = sorted(scores, key=lambda x: (-x[1], x[0]))
